...
function PatientsDetailsCtrl($scope, $state, $stateParams, $http, $timeout){
    ...
    $http.get(URL + "/patient/"+$stateParams.id + "/activities").success( function(response) {
        $scope.chartConfig = {
            options: {
                chart: { height: 600, type: 'line', zoomType: 'x' },
                xAxis: [{ type: 'datetime', ordinal: true }],
                yAxis: [{ title: { text: 'Aktivitäten der letzten 24 Stunden', style: { color: '#80A3CA' } }, labels: { format: '{value}', style: { color: '#80A3CA' } } }],
                useHighStocks: true,
                credits: { enabled: false }
            },
            series: [{ data: response, name: "Distanz in m", color: '#FFA500' }],
            title: { text: 'Aktivitäten von ' + $scope.patient.firstname + " " + $scope.patient.lastname }
        }
    });
    
    $scope.addDrug = function(){
        $http.post(URL + "/patient/drug/add", { id: $stateParams.id, name: $scope.drug.name }).success(function (data) {
            $scope.drugs.push({'id': data.id, 'name': $scope.drug.name});
            $scope.drug = "";
        });
    };
    $scope.deleteDrug = function(index){
        $http.post(URL + "/patient/drug/remove", { id: $scope.drugs[index].id , name: $scope.drug.name }).success(function (data) {
            $scope.drugs.splice(index,1);
        });
    };
    
    $scope.addDisease = function(){
        $http.post(URL + "/patient/disease/add", { id: $stateParams.id, name: $scope.disease.name }).success(function (data) {
            $scope.diseases.push({'id': data.id, 'name': $scope.disease.name});
            $scope.disease = "";
        });
    }
    $scope.deleteDisease = function(index){
        $http.post(URL + "/patient/disease/remove", { id: $scope.diseases[index].id }).success(function (data) {
